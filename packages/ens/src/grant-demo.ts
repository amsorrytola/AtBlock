/**
 * Demo: grant agent spend roles via EAC on Sepolia (setup before revoke).
 *
 * Usage:
 *   ENS_RPC_URL=... ENS_PRIVATE_KEY=0x... ENS_EAC_CONTRACT=0x... \
 *   ENS_AGENT_NAME=buyer.atblock.eth ENS_AGENT_ACCOUNT=0x... \
 *   npm run grant-demo -w @atblock/ens
 */
import type { Address, Hex } from "viem";
import { agentHasRoles, grantAgentRoles, pinStatus, resourceFromName } from "./index.js";
...
async function main() {
  ...
  console.log("pin before:", JSON.stringify(pin, null, 2));
  ...
  const hash = await grantAgentRoles({
    rpcUrl,
    privateKey,
    eacContract,
